<template>
  <view class="container">
      ...
      <view class="form-field">
        ...
        <view class="picker-container">
          <picker 
            ...
            class="picker"
          >
            <nut-input :value="timePeriodOptions[timePeriodIndex]" placeholder="Placeholder" />
          </picker>
    ...
    <view class="footer safe-area">
      ...
      <button @tap="updateTask" class="confirm-btn" :disabled="!isFormValid">
...
const loadTaskData = () => {
  ...
  // 查找任务
  const task = todoStore.tasks.find(t => t.id === taskId.value);
  ...
  taskForm.description = ''; // 任务模型中没有description字段，这里预留
  taskForm.timePeriod = task.timePeriod;
  taskForm.priority = task.priority;
  taskForm.dueDate = task.dueDate;
  
  // 设置时间周期索引
  timePeriodIndex.value = timePeriodValues.indexOf(task.timePeriod);
  
  // 设置截止日期值
  if (task.dueDate) {
    dueDateValue.value = new Date(task.dueDate).toISOString().split('T')[0];
  }
};
